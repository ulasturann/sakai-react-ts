...
const MediaDemo = () => {
    ...
    const [images, setImages] = useState<API.Photo[]>([]);
    ...
    const carouselItemTemplate = (product: API.Product) => {
        return (
    ...
    const galleriaItemTemplate = (item: API.Photo) => <img src={`${contextPath}/${item.itemImageSrc}`} alt={item.alt} style={{ width: '100%', display: 'block' }} />;
    const galleriaThumbnailTemplate = (item: API.Photo) => <img src={`${contextPath}/${item.thumbnailImageSrc}`} alt={item.alt} style={{ width: '100%', display: 'block' }} />;
